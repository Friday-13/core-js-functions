...
function getPolynom(...coefficients) {
  if (coefficients.length !== 0) {
    return function f(x) {
      let polynomDegree = coefficients.length;
      const polynom = coefficients.reduce((y, coeff) => {
        polynomDegree -= 1;
        return y + coeff * x ** polynomDegree;
      }, 0);
      return polynom;
    };
  }
...
function retry(func, attempts) {
  ...
  return function r() {
    ...
    try {
      ...
      lastAttempts -= 1;
      if (lastAttempts < 0) return undefined;
      return r();
    }
  };
}
...
function logger(func, logFunc) {
  return function wrapper(...args) {
    ...
    const functionInfo = `${func.name}(${argsString})`;
    logFunc(`${functionInfo} starts`);
    const result = func(...args);
    logFunc(`${functionInfo} ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function fnPartial(...argsRest) {
    return fn(...args1, ...argsRest);
  };
}
